...
WHERE O.CUSTID = C.CUSTID
    AND SALEPRICE > (SELECT AVG(SALEPRICE) FROM ORDERS) --중첩 서브쿼리(Nested Sub Query)
;
--===================================
-- 박지성이 구입한 내역을 검색
...
--+) 박지성 이름도 같이 
SELECT O.*,(SELECT NAME FROM CUSTOMER WHERE NAME = '박지성')
FROM ORDERS O
WHERE CUSTID =(SELECT CUSTID FROM CUSTOMER WHERE NAME = '박지성');

--조인문 처리 
SELECT C.NAME, O.* FROM ORDERS O, CUSTOMER C --조인테이블
...
--WHERE 절에서 서브쿼리 사용시 서브쿼리 조회 결과가 2건 이상인 경우 IN  사용 
 SELECT * FROM ORDERS 
 WHERE CUSTID IN (SELECT CUSTID FROM CUSTOMER  --> 찾는건수가 여러개일때! IN 사용 주의 
                  WHERE NAME IN('박지성', '김연아'));
                  
-------------
--(서브쿼리) 출판된 책 중 정가가 가장 비싼 도서의 이름을 구하시오
...
;
-------
--(실습)김연아,박지성 구입내역(서브쿼리 -FROM 절)
SELECT * FROM ORDERS O,
        (SELECT NAME FROM CUSTOMER WHERE NAME IN ('김연아','박지성')); 

SELECT O.ORDERID, O.SALEPRICE, O.ORDERDATE, C.NAME  FROM ORDERS O,
        (SELECT NAME FROM CUSTOMER WHERE NAME IN ('김연아','박지성')) C
        ; 
--서브쿼리
SELECT * FROM ORDERS O,(SELECT * FROM CUSTOMER WHERE NAME IN ('김연아', '박지성')) C
WHERE O.CUSTID = C.CUSTID;
--조인문
SELECT * FROM ORDERS O, CUSTOMER C WHERE O.CUSTID = C.CUSTID AND C.NAME IN ('김연아','박지성');
-------
SELECT * FROM ORDERS;
SELECT NAME FROM CUSTOMER WHERE NAME IN('김연아','박지성');
SELECT * FROM ORDERS,(SELECT NAME FROM CUSTOMER WHERE NAME IN ('김연아','박지성'));
----------------
--SELECT 절에 서브쿼리 사용
SELECT * FROM ORDERS;
SELECT ORDERID, CUSTID, BOOKID,(SELECT NAME FROM CUSTOMER 
WHERE ORDERS.CUSTID = CUSTOMER.CUSTID) CUST_NAME FROM ORDERS;
--ALIAS 사용
...
SALEPRICE, ORDERDATE FROM ORDERS O ;

--------
--박지성이 구매한 책 목록
SELECT * FROM BOOK WHERE BOOKID IN
(SELECT BOOKID FROM ORDERS WHERE CUSTID IN
(SELECT CUSTID FROM CUSTOMER WHERE NAME IN ('박지성')));
--===========================================
--(실습) 서브쿼리 이용
...
ON C.CUSTID = O.CUSTID WHERE O.ORDERID IS NOT NULL;
...
--2. 20000원 이상되는 책을 구입한 고객 명단 조회
SELECT * FROM CUSTOMER C,(SELECT * FROM ORDERS WHERE SALEPRICE >= 20000) O
...
(SELECT BOOKID FROM BOOK WHERE PUBLISHER IN ('대한미디어')));

SELECT * FROM CUSTOMER C
INNER JOIN ORDERS O ON C.CUSTID = O.CUSTID 
INNER JOIN BOOK B ON B.BOOKID = O.BOOKID
WHERE B.PUBLISHER IN('대한미디어');

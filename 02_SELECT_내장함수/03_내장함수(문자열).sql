...
SELECT * FROM BOOK WHERE BOOKNAME LIKE 'Olympic Champions';
SELECT * FROM BOOK WHERE BOOKNAME LIKE 'Olympic%';
SELECT * FROM BOOK WHERE BOOKNAME LIKE 'OLYMPIC%'; -- x
SELECT * FROM BOOK WHERE BOOKNAME LIKE 'olympic%'; -- x
...
--SUBSTR(대상, 시작위치, 길이) : 문자열의 일부 추출
  -- (시작위치는 1부터 시작, 오른쪽에서 시작하는 경우 마이너스(-)값 사용)
SELECT SUBSTR('홍길동123', 1) FROM DUAL; -- 홍길동 123: 첫글자부터 끝까지 
